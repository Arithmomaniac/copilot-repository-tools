...
def serve(
    ...
    Note: This requires the copilot-repository-tools-web package to be installed.
    You can run the web server directly with: uvx copilot-repository-tools-web
    """
    try:
        from copilot_repository_tools_web import run_server
    except ImportError:
        console.print("[red]Error: Web package not installed.[/red]")
        console.print("Install with: [cyan]uv add copilot-repository-tools-web[/cyan]")
        console.print("Or run directly: [cyan]uvx copilot-repository-tools-web[/cyan]")
        raise typer.Exit(1)
    ...
    if stats["session_count"] == 0:
        console.print("[yellow]Warning: Database is empty. Run 'copilot-chat-archive scan' first.[/yellow]")
    ...
    console.print(f"\nOpen [link=http://{host}:{port}/]http://{host}:{port}/[/link] in a browser to view your archive.")
    console.print("Press Ctrl+C to stop the server.\n")

    run_server(host=host, port=port, db_path=str(db), title=title, debug=debug)
